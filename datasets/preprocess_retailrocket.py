"""
https://github.com/jin530/SLIST/blob/main/preprocessing/preprocess_retailrocket.py
"""
import numpy as np
import pandas as pd
...
def preprocess_org(path=DATA_PATH, file=DATA_FILE, path_proc=DATA_PATH_PROCESSED, min_item_support=MIN_ITEM_SUPPORT,
    ...
    preprocess_data(data, path_proc + file)
...
def load_data(file):
    ...
    data.columns = ['timestamp', 'user_id', 'Type', 'item_id']
    data['timestamp'] = (data.timestamp / 1000).astype(int)

    data.sort_values(['user_id', 'timestamp'], ascending=True, inplace=True)

    # sessionize
    data['TimeTmp'] = pd.to_datetime(data.timestamp, unit='s')

    data.sort_values(['user_id', 'TimeTmp'], ascending=True, inplace=True)
    ...
    data['session_id'] = data['SessionIdTmp'].cumsum(skipna=False)
    ...
    data_start = datetime.fromtimestamp(data.timestamp.min(), timezone.utc)
    data_end = datetime.fromtimestamp(data.timestamp.max(), timezone.utc)

    del data['TimeTmp']

    print('Loaded data set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}\n\tSpan: {} / {}\n\n'.
          format(len(data), data.session_id.nunique(), data.item_id.nunique(), data_start.date().isoformat(),
...
def filter_data(data, min_item_support=MIN_ITEM_SUPPORT, min_session_length=MIN_SESSION_LENGTH):
    ...
    data = data[np.in1d(data.item_id, item_supports[item_supports >= min_item_support].index)]
...
def preprocess_data(data, output_file):
    ...
    print('Full train set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}'.format(len(train), train.session_id.nunique(),
